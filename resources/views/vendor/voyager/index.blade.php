...
@section('content')
    <div class="page-content">
        ...
        @php
            $user_responsavel_estoque = false;
            $user_administrador_estoque = false;
            if(auth()->user()->role_id == 1 || auth()->user()->role_id == 4){
                $user_responsavel_estoque = true;
                $pedidos = \App\Models\Pedido::where('status', 'Pendente')->get();

            }
            if(auth()->user()->role_id == 1 || auth()->user()->role_id == 2 || auth()->user()->role_id == 4){
                $user_administrador_estoque = true;
                $estoque_vencidos = \App\Models\Estoque::where('validade', '<=', now())->where('quantidade', ">", 0)->get();
                $estoque_perto_vencers = \App\Models\Estoque::where('validade', '<=', now()->addDays(30))
                ->where('quantidade', ">", 0)
                ->where('validade', '>', now())
                ->get();
                $estoque_total = \App\Models\Estoque::with("produto")
                ->get();
                $produto_quantidade = [];
                $produto_quantidade_minima = [];
                foreach($estoque_total as $estoque){
                    $produto_quantidade[$estoque->produto->id] = ($produto_quantidade[$estoque->produto->id] ?? 0) + $estoque->quantidade;
                }
                foreach($produto_quantidade as $produto_id => $quantidade){
                    $produto = \App\Models\Produto::find($produto_id);
                    if($produto){
                        if($quantidade < $produto->quantidade_minima){
                            $produto->quantidade_total = $quantidade;
                            $produto_quantidade_minima[] = $produto;
                        }
                    }
                }
        ...
        @endphp
        @if($user_responsavel_estoque)
            @foreach($pedidos as $pedido)
                @if(isset($pedido->estoque->produto) && isset($pedido->estoque))
                    <div class="alert alert-warning" role="alert">
                        <strong>Pedido Pendente:</strong> Produto: {{ $pedido->estoque->produto->name }} Lote: {{ $pedido->estoque->lote }} Quantidade Solicitada: {{ $pedido->quantidade }} está pendente de entrega. <a href="{{ route('voyager.pedidos.edit', $pedido->id) }}" class="alert-link">Clique aqui para gerenciar.</a>
                    </div>
                @endif
            @endforeach
        @endif
        @if($user_administrador_estoque)
            @foreach($estoque_vencidos as $estoque_vencido)
                <div class="alert alert-danger" role="alert">
                    <strong>Lote Vencido</strong> Produto: {{ $estoque_vencido->produto->name }} Lote: {{ $estoque_vencido->lote }} está vencido. <a href="{{ route('voyager.estoques.show', $estoque_vencido->id) }}" class="alert-link">Clique aqui para gerenciar.</a>
                </div>
            @endforeach
        @endif
        @if($user_administrador_estoque)
            @foreach($estoque_perto_vencers as $estoque_perto_vencer)
                <div class="alert alert-warning" role="alert">
                    <strong>Lote Perto de vencer</strong> Produto: {{ $estoque_perto_vencer->produto->name }} Lote: {{ $estoque_perto_vencer->lote }} vence dia {{ \Carbon\Carbon::parse($estoque_perto_vencer->validade)->format("d/m/Y") }}. <a href="{{ route('voyager.estoques.show', $estoque_perto_vencer->id) }}" class="alert-link">Clique aqui para gerenciar.</a>
                </div>
            @endforeach
        @endif
        @if($user_administrador_estoque)
            @foreach($produto_quantidade_minima as $produto)
                <div class="alert alert-danger" role="alert">
                    <strong>Produto:</strong> {{ $produto->name }} esta com quantidade {{ $produto->quantidade_total }} o ideal seria {{ $produto->quantidade_minima }}. <a href="/admin/estoques" class="alert-link">Clique aqui para gerenciar.</a>
                </div>
            @endforeach
        @endif
    </div>
